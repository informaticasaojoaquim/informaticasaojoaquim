...
<head>
  ...
  <style>
      ...
      /* CONFIGURAÇÃO DE IMPRESSÃO (PDF) - MELHORADA */
      @media print {
          ...
          /* Garante que abas ocultas que contém dados do aluno apareçam na impressão */
          .tab-view { display: block !important; } 
          .hidden:not(.print-visible) { display: none !important; }
          ...
          .month-box { border: 1px solid black !important; background: white !important; color: black !important; }
          .wk-btn { border: 1px solid #ccc !important; }
          h2, h3 { color: black !important; text-align: center; margin-bottom: 20px; }
          .card { page-break-inside: avoid; border: 1px solid #000 !important; margin-bottom: 30px !important; }
      }
  </style>
</head>
<body>
...
<div class="main-container">
  ...
  <main class="content" id="print-area">
    ...
    <div id="tab-matricula" class="tab-view hidden">
      ...
      <div class="card glass no-print">
          <h3>📢 Editar Mural do Dashboard</h3>
          <div style="display: flex; gap: 10px;">
              <input type="text" id="edit-mural-input" placeholder="Digite a nova mensagem do Dashboard...">
              <button class="btn btn-primary" onclick="atualizarMural()">ATUALIZAR MURAL</button>
          </div>
      </div>

      <div class="card glass">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <h3>📋 Relatório Detalhado (Matrícula + Frequência + Notas)</h3>
    ...
    <div id="tab-aluno" class="tab-view hidden">
      <div class="card glass">
        ...
        <h4 id="al-nota-display" style="color: var(--primary);">NOTA: 0.0</h4>
        <hr>
    ...
    <div id="tab-senha" class="tab-view hidden">
        <div class="card glass" style="max-width: 400px; margin: 0 auto;">
            <h3>🔐 Alterar Minha Senha</h3>
            <input type="password" id="pw-atual" placeholder="Senha Atual">
            <input type="password" id="pw-nova" placeholder="Nova Senha">
            <input type="password" id="pw-confirma" placeholder="Confirme a Nova Senha">
            <button class="btn btn-primary" style="width: 100%;" onclick="alterarSenhaPropria()">ATUALIZAR SENHA</button>
        </div>
    </div>
...
<script>
  ...
  function autenticar() {
    ...
    else if(r === 'aluno') { const a = db.alunos.find(x => x.user === u && x.pass === p); if(a) logar('aluno', a); else alert("Usuário ou Senha incorretos!"); }
    else { const f = db.funcionarios.find(x => x.user === u && x.pass === p && x.role === r); if(f) logar(r, f); else alert("Dados inválidos!"); }
  }
  ...
  function logar(role, dados) {
    ...
    document.getElementById('edit-mural-input').value = db.mural;
  }
  ...
  function montarMenu() {
    ...
    if(sessao.role !== 'aluno') {
      ...
      h += `<div class="menu-item" onclick="showTab('tab-frequencia')">Frequência e Notas</div>`;
  ...
  function atualizarMural() {
      const novoTexto = document.getElementById('edit-mural-input').value;
      if(novoTexto.trim() !== "") {
          db.mural = novoTexto;
  ...
  function cadastrarAluno() {
    ...
    if(!nome || !user) return;
    // Adicionado campo 'nota' no objeto aluno
    db.alunos.push({ id: Date.now(), nome, curso, user, pass, tel, resp, endereco, nota: 0, freq: Array(12).fill().map(() => Array(4).fill('P')) });
    salvar(); renderizarGeral();
    ['m-nome','m-curso','m-user','m-tel','m-resp','m-end'].forEach(id => document.getElementById(id).value = "");
  }

  function altNota(aid, val) {
      const a = db.alunos.find(x => x.id === aid);
      if(a) { a.nota = val; salvar(); }
  }
  ...
  function renderizarGeral() {
    ...
    document.getElementById('list-alunos').innerHTML = db.alunos.map(a => `
                <tr>
                    <td>${a.nome}</td><td>${a.curso}</td><td>${a.tel}</td><td>${a.resp}</td>
                    <td><b>${a.nota || 0}</b></td>
    ...
    // Prepara a área de impressão de frequência (invisível na tela, visível no PDF)
    document.getElementById('freq-print-area').innerHTML = db.alunos.map(a => `
        <div class="card" style="margin-top:20px; border:1px solid #000;">
            <h4>Frequência Detalhada: ${a.nome} (Nota: ${a.nota || 0})</h4>
            <div class="freq-row">${a.freq.map((m, im) => `
                <div class="month-box"><small>${MESES[im]}</small><br>
                ${m.map(s => `<div class="wk-btn ${s.toLowerCase()}-bg">${s}</div>`).join('')}
                </div>`).join('')}
            </div>
        </div>
    `).join('');
    ...
    document.getElementById('freq-area').innerHTML = db.alunos.map(a => `
        <div class="card glass">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                <b>${a.nome}</b>
                <div>Nota: <input type="number" step="0.1" value="${a.nota || 0}" style="width:70px; display:inline;" onchange="altNota(${a.id}, this.value)"></div>
            </div>
            <div class="freq-row">${a.freq.map((m, im) => `
                <div class="month-box"><small>${MESES[im]}</small><br>
  ...
  function altF(aid, im, is) { const a = db.alunos.find(x => x.id === aid); const c = {'P':'F', 'F':'FJ', 'FJ':'P'}; a.freq[im][is] = c[a.freq[im][is]]; salvar(); renderFreq(); renderizarGeral(); }
  ...
  function renderPortalAluno() {
    ...
    document.getElementById('al-nota-display').innerText = "NOTA: " + (a.nota || 0);
  ...
  function alterarSenhaPropria() {
      ...
      if(nova !== confirma) { alert("Senhas não coincidem!"); return; }
      let alterado = false;
      if(sessao.role === 'admin') { if(atual === db.admin.pass) { db.admin.pass = nova; alterado = true; } } 
      else if(sessao.role === 'aluno') { const a = db.alunos.find(x => x.id === sessao.id); if(a && atual === a.pass) { a.pass = nova; alterado = true; } } 
      else { const f = db.funcionarios.find(x => x.user === sessao.user); if(f && atual === f.pass) { f.pass = nova; alterado = true; } }
      if(alterado) { salvar(); alert("Senha alterada!"); location.reload(); } else alert("Senha atual incorreta.");
  }
